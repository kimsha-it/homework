...
async function getCommentByID(n) {
  const response = await axios.get(`${BASE_URL}/comments/${n}`);
  console.log(response.data);
}
getCommentByID(6);
/* 출력 결과
...
async function getRecipes() {
  fetch(`${BASE_URL}/recipes/tag/beef`)
...
async function getProductCate() {
  fetch(`${BASE_URL}/products?sortBy=price&order=desc`)
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
    });
